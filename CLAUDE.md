...
- **office-hours/SKILL.md.tmpl:** accept upstream's version, then re-remove Phase 4.5
  (Signal Synthesis), Phase 6 (Handoff — Discovery), and YC branding.
...
1. **Touches ETHOS.md** — preserve upstream philosophy unless the change is part
   of an explicit fork-sanitization pass.
2. **Reintroduces promotional material** — YC references, founder-first framing,
   hiring CTAs, and personal-branding copy should stay out of this fork unless
   explicitly requested.
3. **Changes the skill voice contract** — direct, concrete, builder-to-builder is
   intentional. Do not replace it with corporate or generic assistant prose.

When syncing upstream, keep workflow improvements and strip branding regressions.
If in doubt, ask before reintroducing promotional material.
