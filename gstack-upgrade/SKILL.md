...
elif [ -f "$HOME/.gstack/hermes-source" ]; then
  INSTALL_TYPE="hermes"
  INSTALL_DIR=$(cat "$HOME/.gstack/hermes-source" 2>/dev/null)
  [ -d "$INSTALL_DIR/.git" ] || INSTALL_DIR=""
  if [ -z "$INSTALL_DIR" ]; then
    # Source repo gone — clone fresh
    mkdir -p "$HOME/.gstack/repos"
    git clone --depth 1 https://github.com/donovan-yohan/gstack-adfree.git "$HOME/.gstack/repos/gstack"
    INSTALL_DIR="$HOME/.gstack/repos/gstack"
    echo "$INSTALL_DIR" > "$HOME/.gstack/hermes-source"
  fi
...
./setup --host hermes
```
If `$STASH_OUTPUT` contains "Saved working directory", warn the user: "Note: local changes were stashed. Run `git stash pop` in the source directory to restore them."
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/donovan-yohan/gstack-adfree.git "$TMP_DIR/gstack"
