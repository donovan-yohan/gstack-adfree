...
Once the design doc is approved, point the user at the right downstream skill:

- `/gstack-openclaw-ceo-review` for ambitious features — rethink the problem, find the best product shape
- Direct implementation for well-scoped design docs — the design is in `memory/` and downstream skills can read it
